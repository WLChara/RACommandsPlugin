...
这是一个独立的 Win32 C++ 注入式 DLL 项目，逐步封装《红色警戒 2：尤里的复仇》的原生命令接入。当前版本仅面向特定的 Mental Omega `gamemd.exe` 样本：注入后验证 EXE SHA-256、解析命令符号，在游戏主帧注册「自动装车」原生命令，并通过直接 `ClickedMission` 尝试装车。真实游戏注入和联机行为尚未验收。
