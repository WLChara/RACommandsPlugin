...
// ******************* START OPERATION FUNCTIONS ******************* //
	...
	//修复这个建筑
	bool Repair() {
		return ClickedEvent(NetworkEvents::Repair);
	}

	//operation sell
	//卖掉这个建筑
	bool Sell() {
		return ClickedEvent(NetworkEvents::Sell);
	}

	//operation power-on
	//打开电力开关
	bool PowerOn() {
		return ClickedEvent(NetworkEvents::PowerOn);
	}

	//operation power-off
	//关闭电力开关
	bool PowerOff() {
		return ClickedEvent(NetworkEvents::PowerOff);
	}
