...
struct VeterancyStruct
{
	...
	void SetRookie(bool notReally = false) noexcept {
...
public:
	...
	//2026 05 13 ： 这个pTarget可以是任何继承了AbstractClass的类，比如CellClass，之前的定义不准确
	//2026 05 13 ： 比如你想要攻击一个格子，你应该在pTarget里面传一个CellClass*
	//2026 05 13 ： TargetCell和NearestTargetCellICanEnter的定义似乎也不准确。。。实测中，工程师修复建筑时，CellClass*传的参是个BuildingClass*，给我逗笑了
	//2026 05 13 ： 命名就先这么留着，以后再更改吧
	virtual bool ClickedMission(Mission Mission, AbstractClass *pTarget, AbstractClass * TargetCell, CellClass *NearestTargetCellICanEnter) R0; //378
...
// ******************* START LOCALDEFINED FUNCTIONS ******************* //


	//单位是否在建筑上面。比如：坦克在重工边缘上面
	bool IsAtBuilding() {
		auto cell = this->GetCell();
		if (!cell) {
			return false;
		}
...
// ******************* START OPERATION FUNCTIONS ******************* //
	...
	//operation attack
	//攻击单位或格子
	bool Attack(AbstractClass* pAttackTarget)
	{
		if (!pAttackTarget)
		{
			return false;
	...
	//operation enter
	//进入一个单位或格子，不建议进入步兵，因为效果还没测试
	bool Enter(AbstractClass* pEnterTarget)
	{
		if (!pEnterTarget)
		{
			return false;
	...
	//operation move
	//移动到
	bool Move(AbstractClass* pCellTarget)
	{
		if (!pCellTarget)
		{
			return false;
	...
	//operation ambush move
	//保持埋伏状态移动到一点   待测
	bool AmbushMove(CellClass* pTargetCell) {
		return this->ClickedMission(
			Mission::Ambush,
			nullptr,
			pTargetCell,
	...
	//operation attackmove
	//移动攻击，Ctrl + Shift
	bool AttackMove(AbstractClass* pTarget) {
		return this->ClickedMission(
			Mission::AttackMove,
			nullptr,
			pTarget,
	...
	//operation capture
	//好像是占领？
	bool Capture(AbstractClass* pTarget) {
		return this->ClickedMission(
			Mission::Capture,
			nullptr,
			pTarget,
	...
	//operation eaten
	//鬼知道Eaten是什么玩意
	bool Eaten(CellClass* pTargetCell) {
		return this->ClickedMission(
			Mission::Eaten,
			nullptr,
			pTargetCell,
	...
	//operation harvest
	//去一个地点采矿去
	bool Harvest(CellClass* pTargetCell) {
		return this->ClickedMission(
			Mission::Harvest,
			nullptr,
			pTargetCell,
	...
	//operation harmless
	//让单位运动到一点，期间不受到任何敌友单位攻击
	bool Harmless(CellClass* pTargetCell) {
		return this->ClickedMission(
			Mission::Harmless,
			nullptr,
			pTargetCell,
	...
	//operation abandon
	//建筑物弃用，比如动员兵从战斗碉堡走出来，对载具用的话和deploy差不多效果，但是对步兵似乎是不一样的（待测）
	bool Abandon() {
		return this->ClickedEvent(NetworkEvents::Abandon);
	}

	//operation scatter
	//散开，分散
	bool Scatter() {
		return this->ClickedEvent(NetworkEvents::Scatter);
	}

	//operation idle
	//停下来然后什么也不干
	bool Idle() {
		return this->ClickedEvent(NetworkEvents::Idle);
	}

	//operation detonate
	//引爆身上的炸弹
	bool Detonate() {
		return this->ClickedEvent(NetworkEvents::Detonate);
	}

	//operation sellcell
	//出售一个格子上面的单位，前提是这个格子是建筑的一部分
	//这个函数自己内置了一个判断单位当前所在格有没有建筑的逻辑，如果你希望直接卖，自己调用clicknetworkev版本去
	bool SellCell() {
		auto cell = this->GetCell();
		if (!cell->GetBuilding()) {
			return false;
		}
		return ClickedEvent(NetworkEvents::SellCell);
	}
