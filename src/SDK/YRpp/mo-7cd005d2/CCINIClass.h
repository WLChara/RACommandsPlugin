...
#define INI_READ(item, addr) \
	...
	ColorStruct* ReadColor(ColorStruct* pBuffer, const char* pSection, const char* pKey, ColorStruct const& _default)
		{ JMP_THIS(0x474C70); }

	ColorStruct ReadColor(const char* const pSection, const char* const pKey, ColorStruct const& _default) {
		ColorStruct outBuffer;
		this->ReadColor(&outBuffer, pSection, pKey, _default);
		return outBuffer;
	}
