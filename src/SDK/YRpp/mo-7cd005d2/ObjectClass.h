...
public:
  ...
  */
	...
	virtual bool IsStandingStill() const R0;        //BC
	virtual bool IsDisguised() const R0;     //C0
	virtual bool IsDisguisedAs(HouseClass *target) const R0; // only works correctly on infantry!     C4
	virtual ObjectTypeClass* GetDisguise(bool DisguisedAgainstAllies) const R0;    //C8
	virtual HouseClass* GetDisguiseHouse(bool DisguisedAgainstAllies) const R0;    //CC

	// remove object from the map
	virtual bool Remove() R0;      //D0

	// place the object on the map
	virtual bool Put(const CoordStruct& Crd, Direction::Value dFaceDir) R0;     //D4

	// cleanup things (lose line trail, deselect, etc). Permanently: destroyed/removed/gone opposed to just going out of sight.
	virtual void Disappear(bool permanently) RX;     //D8

	virtual void RegisterDestruction(TechnoClass *Destroyer) RX;  //DC

	 // maybe Object instead of Techno? Raises Map Events, grants veterancy, increments house kill counters           E0
	virtual void RegisterKill(HouseClass *Destroyer) RX; // ++destroyer's kill counters , etc              E4

	virtual bool SpawnParachuted(const CoordStruct& coords) R0;  //E8
	virtual void DropAsBomb() RX; // EC
	virtual void MarkAllOccupationBits(const CoordStruct& coords) RX; // F0
	virtual void UnmarkAllOccupationBits(const CoordStruct& coords) RX; // F4
	virtual void UnInit() RX; // F8
	virtual void Reveal() RX; // uncloak when object is bumped, damaged, detected, ... FC
	virtual KickOutResult KickOutUnit(TechnoClass* pTechno, CellStruct Cell) RT(KickOutResult); //100
	virtual bool DrawIfVisible(RectangleStruct *pBounds, bool EvenIfCloaked, DWORD dwUnk3) const R0;   //104
	...
	virtual void Draw(Point2D* pLocation, RectangleStruct* pBounds) const RX;		//114  draw it
	...
	virtual DWORD GetPointsValue() const R0;   //180
	virtual Mission GetCurrentMission() const RT(Mission);   //184
	virtual void RestoreMission(Mission mission) RX;   //188
	virtual void UpdatePosition(int dwUnk) RX;   //18C
	virtual BuildingClass* FindFactory(bool allowOccupied, bool requirePower) const R0;   //190
	virtual RadioCommand ReceiveCommand(TechnoClass* pSender, RadioCommand command, AbstractClass* &pInOut) RT(RadioCommand);   //194
	virtual bool DiscoveredBy(HouseClass *pHouse) R0;    //198
	virtual void SetRepairState(int state) RX; // 0 - off, 1 - on, -1 - toggle    //19C
	virtual void Sell(DWORD dwCondition) RX; //1 or 0xFFFFFFFF : sell ; else : dont sell    //1A0
	virtual void AssignPlanningPath(signed int idxPath, signed char idxWP) RX;  //1A4
	virtual void vt_entry_1A8(DWORD dwUnk) RX;  //1A8
